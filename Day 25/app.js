...
const mostPopulatedCountries = () => {
  let bool=true;
  if (bool) {
    let result = countries_data.map((v) => (total += v.population));
    bool=false;
  }
  //numbOne.textContent = total;

  let result2 = countries_data.sort((a, b) => {
    return b.population - a.population;
  });
  const arr = result2.slice(0, 12);
  arr.unshift({name:"World",population:7758589152})

  for (let i = 0; i < arr.length; i++) {
    li = liElement = document.createElement("li");
    spanElement = document.createElement("span");
    spanElement2 = document.createElement("span");
    divElement = document.createElement("div");
    spanElement.className = "title";
    if ((spanElement.textContent = arr[i].name == "United States of America")) {
      spanElement.textContent = "USA";
    } else if (
      (spanElement.textContent = arr[i].name == "Russian Federation")
    ) {
      spanElement.textContent = "Russian";
    } else {
      spanElement.textContent = arr[i].name;
    }
    if (arr[i].population > 7000000000) {
      divElement.className = "bar-60";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
      
    }
    if (arr[i].population > 1000000000 && arr[i].population <= 1500000000) {
      divElement.className = "bar-10";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }
    if (arr[i].population > 1000000000 && arr[i].population <= 1400000000) {
      divElement.className = "bar-9";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }

    if (arr[i].population > 300000000 && arr[i].population <= 400000000) {
      divElement.className = "bar-5";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }

    if (arr[i].population > 200000000 && arr[i].population <= 280000000) {
      divElement.className = "bar-4-2";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }

    if (arr[i].population > 200000000 && arr[i].population <= 250000000) {
      divElement.className = "bar-3";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }

    if (arr[i].population > 200000000 && arr[i].population <= 230000000) {
      divElement.className = "bar-3-2";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }
    if (arr[i].population > 200000000 && arr[i].population <= 225000000) {
      divElement.className = "bar-4";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }
    if (arr[i].population > 160000000 && arr[i].population <= 180000000) {
      divElement.className = "bar-3";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }

    if (arr[i].population > 140000000 && arr[i].population <= 150000000) {
      divElement.className = "bar-2";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }
    if (arr[i].population > 120000000 && arr[i].population <= 130000000) {
      divElement.className = "bar-1";
      spanElement2.className = "numb";
      spanElement2.textContent = arr[i].population;
    }
...
populationBtn.addEventListener("click", () => {
  ...
  pElement.textContent="10 Most populated  countries in the world"
});

const mostSpokenLanguages = (countries) => {
  let val = countries.map((x) => x.languages);
  let newArr = [];
  const counts = [];
  ...
  const uniqNewArr = new Set(newArr);
  for (const l of uniqNewArr) {
    const filteredLang = newArr.filter((lng) => lng == l);
    counts.push({ lang: l, count: filteredLang.length });
  }
  let maxToMin = [...counts].sort(function (a, b) {
    return b.count - a.count;
  });
  return maxToMin.slice(0, 10);
};

let val = mostSpokenLanguages(countries_data);

const mostSpeakLanugesInWorld = () => {
  for (let i = 0; i < val.length; i++) {
    li = liElement = document.createElement("li");
    spanElement = document.createElement("span");
    spanElement2 = document.createElement("span");
    divElement = document.createElement("div");
    spanElement.textContent = val[i].lang;
    if (val[i].count>90 && val[i].count<=100) {
      divElement.className = "bar-60";
      spanElement2.className = "numb";
      spanElement.className = "title";
    }
    if (val[i].count>40 && val[i].count<=60) {
      divElement.className = "bar-40";
      spanElement2.className = "numb";
      spanElement.className = "title";
    }
    if (val[i].count>20 && val[i].count<=30) {
      divElement.className = "bar-10";
      spanElement2.className = "numb";
      spanElement.className = "title";
    }

    if (val[i].count>6 && val[i].count<=12) {
      divElement.className = "bar-5";
      spanElement2.className = "numb";
      spanElement.className = "title";
    }
    if (val[i].count>1 && val[i].count<=5) {
      divElement.className = "bar-3";
      spanElement2.className = "numb";
      spanElement.className = "title";
    }
    spanElement2.textContent = val[i].count;
    li.appendChild(spanElement);
    li.appendChild(divElement);
    li.appendChild(spanElement2);
    ulElement.appendChild(li);
...
languagesBtn.addEventListener("click", () => {
  if (ulElement.textContent == "") {
    mostSpeakLanugesInWorld();
  } else {
    ulElement.textContent = "";
    mostSpeakLanugesInWorld();
  }
  pElement.textContent="";
  pElement.textContent="10 Most Spoken languages in the world";
});
